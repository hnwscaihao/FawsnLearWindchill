(function ($) {
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        this.$body.find('> tr[data-index]').each(function () {
            ...
            for (var i = 0; i < end; i++) {
            	/** Modify By Cai Hao : 解决内容单元格和固定列冲突问题*/
    ...
    BootstrapTable.prototype.fitHeaderColumns = function () {
        ...
        this.$body.find('tr:first-child:not(.no-records-found) > *').each(function (i) {
             ...
             * 
             */
            var preThOpt = $this.parent().parent().prev().find('th[data-field="' + visibleFields[index] + '"]');
            var fixedThOpt = that.$fixedHeader.find('th[data-field="' + visibleFields[index] + '"]');
            var styles = preThOpt[0].style;//复制Style 样式
            if(styles){
            	for(var i=0; i<styles.length; i++){
            		var styleName = styles[i];
            		fixedThOpt.css(styleName, preThOpt.css(styleName));
            	}
            }
            fixedThOpt.height(preThOpt.height());
            fixedThOpt.find('.fht-cell').width($this.innerWidth());
